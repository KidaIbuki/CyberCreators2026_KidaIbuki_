...
float4x4 WorldViewProj;     // ワールド→ビュー→プロジェクション一括変換用行列
float3x3 gWorldInverseTranspose;    // 法線変換用の逆転置行列

float3 LightDirection = normalize(float3(0.5f, -1.0f, 0.5f)); // 平行光源の方向
...
VS_OUTPUT VS_Main(VS_INPUT input)
{
    ...
    // ライト方向との内積を計算
    float NdotL = dot(worldNormal, -LightDirection); // 光の逆方向を見る
    output.LightIntensity = saturate(NdotL); // 0~1 にする
...
float4 PS_Main(VS_OUTPUT input) : COLOR
{
    ...
    float lighting = step(0.5, input.LightIntensity); // 0.5未満は影
